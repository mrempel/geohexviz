...
Second, the user can generate a visualization via importing and invoking functions found in the GeoHexViz's Python modules. When using the second method, the reference to the data may be a DataFrame or GeoDataFrame object. If the input reference is a GeoDataFrame, the package does not *need* latitude or longitude columns. Rather, the input to the software will be the entries within the geometry column (it is up to the user to ensure that these are valid geometry types).
